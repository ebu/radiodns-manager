                ...
                for c in self.channels:
                    if c not in self.channels_last_message or (self.channels_last_message[c] + config.FB_FALLBACK_TIME < time.time()):
                        ...
                        if not details:
                            ...
                            headers = {'trigger-time': 'NOW', 'trigger-Time': 'NOW', 'Trigger-Time': 'NOW', 'RadioVIS-Trigger-Time': 'NOW', 'link': link, 'message-id': str(uuid.uuid4().int), 'topic': c + 'text', 'expires': '0', 'priority': '0', 'timestamp': timestamp}
                            self.rabbitmq.send_message(headers, "TEXT " + text)

                            headers = {'trigger-time': 'NOW', 'trigger-Time': 'NOW', 'Trigger-Time': 'NOW', 'RadioVIS-Trigger-Time': 'NOW', 'link': link, 'message-id': str(uuid.uuid4().int), 'topic': c + 'image', 'expires': '0', 'priority': '0', 'timestamp': timestamp}
