...
from apps.awsutils.awsutils import get_or_create_bucket
from apps.si_pi.si_pi_serving_provider.base_serving import BaseServing, EVENT_SI_PI_UPDATED, EVENT_SI_PI_DELETED
from apps.si_pi.si_pi_serving_provider.utilities import generate_si_file, generate_pi_file
from config.settings.base import SP_PI_BUCKET_NAME, SP_PI_CLOUDFRONT_DOMAIN, API_SECRET


class AWSServing(BaseServing):
    """
    Implements spi file serving for AWS CloudFront integration. On event will upload/delete the SI/PI files to an s3 bucket and
    on request will redirect with a 304 to the bucket.
    """

    def __init__(self):
        bucket, _ = get_or_create_bucket(SP_PI_BUCKET_NAME)
        self.bucket = bucket

    def on_si_resource_changed(self, event_name, service_provider, client=None):
        if event_name == EVENT_SI_PI_UPDATED:
            self.upload_file(
                AWSServing.get_static_bucket_si_filename(service_provider.codops, "1",
                                                         client.identifier if client else "default"),
                generate_si_file(service_provider, client, "servicefollowing/xml1.html")
            )
            self.upload_file(
                AWSServing.get_static_bucket_si_filename(service_provider.codops, "3",
                                                         client.identifier if client else "default"),
                generate_si_file(service_provider, client, "servicefollowing/xml3.html")
            )
        elif event_name == EVENT_SI_PI_DELETED:
    ...
    def on_pi_resource_changed(self, event_name, station):
        ...
        if event_name == EVENT_SI_PI_UPDATED:
            ...
            for i in range(0, 14):  # RadioEPG 1.1
                ...
                contents = generate_pi_file(start_date, "/templates/radioepg/schedule/xml1.html", station=station)
                if not contents:
                    continue
                ...
                for channel in filtered:
                    try:
                        self.upload_file(
                            AWSServing.get_static_bucket_pi_filename(
                                1,
            ...
            for i in range(0, 14):  # SPI 3.1
                start_date = (start_of_the_week + datetime.timedelta(days=i)).strftime("%Y%m%d")
                contents = generate_pi_file(start_date, "/templates/radioepg/schedule/xml3.html", station=station)
                if not contents:
                    continue
    ...
    def on_request_xsi_1(self, codops, client_identifier):
        return redirect(
            AWSServing.get_file_url({"codops": codops, "version": "1", "client_identifier": client_identifier}),
            code=301)

    def on_request_si_3(self, codops, client_identifier):
        return redirect(
            AWSServing.get_file_url({"codops": codops, "version": "3", "client_identifier": client_identifier}),
            code=301)

    def on_request_pi_1(self, request, path, date):
        return redirect(AWSServing.get_file_url({"path": path, "date": str(date), "version": 1}, type="pi"),
                        code=301)

    def on_request_pi_3(self, request, path, date):
        return redirect(AWSServing.get_file_url({"path": path, "date": str(date), "version": 3}, type="pi"),
                        code=301)
    ...
    def get_file_url(data, type="si"):
        ...
        """
        if type == "si":
            return "https://" + SP_PI_CLOUDFRONT_DOMAIN + "/" + AWSServing.get_static_bucket_si_filename(
                data["codops"], data["version"], data["client_identifier"])
        elif type == "pi":
            return "https://" + SP_PI_CLOUDFRONT_DOMAIN + "/" + AWSServing.get_static_bucket_pi_filename(
                data["version"],
                data["path"],
                data["date"],
            )
    ...
    def get_static_bucket_si_filename(service_provider_codops, version, client_identifier):
        ...
        """
        name = (str(service_provider_codops) + str(version) + str(API_SECRET) + str(
