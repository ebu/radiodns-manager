...
def verify_srv_records():
    ...
    for zone in route53.get_zones():
        for record in zone.get_records():
            if record.type == "SRV":
                srv_records.append(
                    (record.name, record.resource_records[0].split(" ")[3], zone.name)
                )
    ...
    for srv_rec in srv_records:
        if srv_rec[1] + "." not in map(lambda x: x[0], second_hosted_zone_cnames):
            raise EnvironmentError(
                ...
                zone cnames records.""".format(
                    name=srv_rec[0], target=srv_rec[1], zone=srv_rec[2]
                )
            )
    ...
    for cname_rec in second_hosted_zone_cnames:
        if get_cnames_until_a_record(second_hosted_zone, cname_rec[1]) is None:
            raise EnvironmentError(
                """The CNAME record {cname_rec} of the second hosted zone isn't pointing on the A record""".format(
                    cname_rec=cname_rec
                )
            )
...
def check_mainzone():
    ...
    return {
        "isvalid": isvalid,
        "name": mainzone.name,
        "mainzone": {"zone": mainzone.name, "ns": mainns},
        "parentns": {
            "entry": "Parent zone.",
...
def check_serviceprovider(sp):
    if sp:
        ...
        return {
            "isvalid": isvalid,
            "name": zone.name,
            "zone": {"isvalid": nsisvalid, "zone": zone.name, "ns": zonens},
            "parentns": {"entry": parent.name, "value": parentns},
            "bucket": {
                "isvalid": bucketisvalid,
                "name": bucket.name,
                "publicendpoint": bucketendpoint,
                "cname": {"name": staticcnamename, "record": staticnamerecord},
            },
            "services": {
                "vis": {"name": sp.vis_fqdn, "value": visservice},
...
def check_station(st):
    if st:
        ...
        return {
            ...
            "radiovis": {
                "isvalid": visisvalid,
                "enabled": st.radiovis_enabled,
                "fqdn": vis,
            },
            "radioepg": {
                "isvalid": epgisvalid,
                "enabled": st.radioepg_enabled,
                "fqdn": epg,
            },
            "radiospi": {
                "isvalid": spiisvalid,
                "enabled": st.radiospi_enabled,
                "fqdn": spi,
            },
            "radiotag": {
                "isvalid": tagisvalid,
                "enabled": st.radiotag_enabled,
                "fqdn": tag,
            },
        }
