...
var radiovisplayer_websockets = [];

function radiovisplayer_initsocket(topic, host, port, secure) {

    let ws = new WebSocket((secure ? "wss" : "ws") + "://" + host + ":" + port + topic);
...
(function ($) {
    $.fn.radiovisplayer = function (topic, host, secure = false) {
        let port = 443;
        ...
        this.find('.radiovis-outtertext').bind('scrollage', function () {
            ...
            $(this)
                ...
                .animate({scrollLeft: 0}, {
                    duration: animSpeed, queue: true, complete: function () {
                        $(this).trigger('scrollage');
                    }
                });
        }).trigger('scrollage');

        radiovisplayer_initsocket(topic, host, port, secure);
