...
DATABASE_CONNECTION_MERCY_TIME = int(
    os.environ.get("DATABASE_CONNECTION_MERCY_TIME", "60")
)


@backoff.on_exception(
    backoff.fibo, psycopg2.OperationalError, max_time=DATABASE_CONNECTION_MERCY_TIME,
)
...
while conn is None:
    try:
        ...
        logging.warning(
            """Couldn't connect to the server in {time} seconds to the database.""".format(
                time=DATABASE_CONNECTION_MERCY_TIME
            )
        )
